...
def list_users():
    ...
    if choice == "S":
        ...
        if player.max_level < 3:
            ...
            return
        if (time_diff := int(time.time()) - player.last_kill) < 60*30:
            input(f"Calm down killer, {30-int(time_diff/60)} mins left for next kill :)")
            return
        target_player_name = input("Target player name: ")
        if target_player_name == player.player_name:
            input("lol, you can't suicide here. If you insist just go fight with lv.10000 cats🐱 :D")
            return
        ...
        if target_player['max_level'] < player.max_level:
